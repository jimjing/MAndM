...
with low-level motion planning when creating robot controllers (\TODO{cite}).
...
Many research show promising results on controller synthesis with formal methods (\TODO{cite}).
...
Many frameworks were developed on controller synthesis with formal methods for manipulation (\TODO{cite}).
Most of these works utilize AI based planners for high-level task planning and low-level motion planners for finding feasible robot trajectories (\TODO{cite}).
...
Moreover, formal methods are also used in robotics to generate controllers for coordination of a team of robots (\TODO{cite}).
